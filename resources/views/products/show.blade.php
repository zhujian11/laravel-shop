  ...
  $(document).ready(function () {
    ...
    $('.btn-crowdfunding').click(function () {
      ...
      $form.append('<div class="form-group row">' +
        ...
        '<div class="col-sm-9">' +
        '<select class="custom-select form-select" name="address_id"></select>' +
        '</div></div>');
      // 循环每个收货地址
      ...
      $form.append('<div class="form-group row mt-3">' +
    ...
    $('.btn-seckill').click(function () {
      ...
      var addresses = {!! json_encode(Auth::check() ? Auth::user()->addresses : []) !!};
      // 使用 jQuery 动态创建一个下拉框
      var addressSelector = $('<select class="form-select"></select>');
      // 循环每个收货地址
      addresses.forEach(function (address) {
        // 把当前收货地址添加到收货地址下拉框选项中
